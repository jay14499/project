...
{
	int res=0;
	dev_t devno=0;          //dev_t is a typedef of int in kernel
	devno=MKDEV(42,0);      //MKDEV is a predefined macro which associates with major number nad minor number
	res=register_chrdev_region(devno,1,"driv_char");//registering driver with vfs layer of kernel
	if(res<0)
	{
		printk(KERN_ALERT "driv_char is not loaded\n");
		return res;
	}
	else
	{
	...
	if(res<0)
	{
		printk(KERN_ALERT "cdev_object is not loaded\n");
...
{
	dev_t devno=MKDEV(42,0); //MKDEV is a predefined macro which associates with major and minor number
